...
namespace network
{
    ...
    size_t client_socket::receive(simple_buffer<char> &buf)
    {
        ...
        size_t read = static_cast<size_t>(assert_ret_code(::recv(raw_fd(), buf.end(), buf.remaining(), 0)));
        buf.push(read);
        return read;
    }
    ...
    {
        size_t sent = static_cast<size_t>(assert_ret_code(::send(raw_fd(), buf.begin(), buf.size(), 0)));
        buf.pop(sent);
        return sent;
    }
